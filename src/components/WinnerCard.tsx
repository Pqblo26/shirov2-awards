...
interface Winner {
    ...
    color: string;
    // --- AÑADIDO ---
    info_url?: string; // URL opcional para el botón/enlace
}
...
// Color styles definition - Includes 'gold'
...
const WinnerCard: React.FC<WinnerCardProps> = ({ winner }) => {
    ...
    // onClick handler no longer needed for simple link
    ...
    return (
        ...
        >
            ...
            {/* --- MODIFICADO: Botón ahora es un enlace condicional --- */}
            {/* Renderiza el enlace solo si winner.info_url tiene un valor */}
            {winner.info_url && (
                <motion.a // Cambiado de button a 'a'
                    href={winner.info_url} // Usa la URL del prop
                    target="_blank" // Abre en nueva pestaña
                    rel="noopener noreferrer" // Buenas prácticas de seguridad
                    className={`mt-4 inline-block ${styles.buttonBg} text-white text-xs px-4 py-1.5 rounded-full shadow ${styles.buttonHoverBg} transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 ${styles.ring} self-center`}
