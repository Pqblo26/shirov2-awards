...
function SingleTranslationPage() {
    ...
    if (isLoading) { return ( <div className="flex justify-center items-center min-h-[50vh]"> <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-12 h-12 border-4 border-t-cyan-500 border-r-cyan-500/30 border-b-cyan-500/30 border-l-cyan-500/30 rounded-full" ></motion.div> </div> ); }
    if (error) { return ( <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center"> <motion.div variants={itemVariants} initial="hidden" animate="visible"> <h1 className="text-2xl font-bold text-red-500 mb-4">Error</h1> <p className="text-gray-400 mb-6">{error}</p> <Link to="/traducciones" className="text-cyan-400 hover:text-cyan-300 hover:underline">Volver a Traducciones</Link> </motion.div> </div> ); }
    if (!translation) { return <div className="text-center py-12 text-gray-500">Traducción no disponible o cargando...</div>; }
    ...
    // --- Define Disqus configuration - ADDED Optional Chaining & Fallbacks ---
    ...
    const disqusConfig = translation && pageUrl ? {
        ...
        title: translation?.title ?? 'Comentarios', // Fallback title
    } : null;
    // --- END MODIFICATION ---
    ...
    return (
        <>
            ...
            >
                {/* ... (Rest of the JSX rendering remains the same as v13/v14) ... */}
                 ...
                 {/* Title */}
                 <motion.h1 variants={itemVariants} className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-10"> {translation.title} </motion.h1>
                 {/* Main Info Section */}
                 <motion.section variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-12"> {/* Image Column */} <div className="lg:col-span-1"> <div className="rounded-lg shadow-xl overflow-hidden border border-gray-700/50"> {translation?.imageUrl ? ( <motion.img src={translation.imageUrl} alt={`Imagen para ${translation.title}`} className="w-full h-auto object-cover aspect-[2/3]" loading="lazy" onError={(e) => { const imgElement = e.target as HTMLImageElement; imgElement.style.display = 'none'; const placeholder = imgElement.parentElement?.querySelector('.placeholder-image'); placeholder?.classList.remove('hidden'); }} /> ) : null} <div className={`placeholder-image w-full h-auto bg-gray-800/50 aspect-[2/3] flex items-center justify-center ${translation?.imageUrl ? 'hidden' : ''}`}> <span className="text-gray-500 italic">Imagen no disponible</span> </div> </div> </div> {/* Details Column */} <div className="lg:col-span-2 bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-lg shadow-xl border border-gray-700/50 p-6"> <h2 className="flex items-center text-xl font-semibold text-white mb-5 border-b border-cyan-500/30 pb-2"> <IconClipboardList className="w-5 h-5 mr-2 text-cyan-400"/> Detalles Técnicos </h2> <div className="grid grid-cols-1 sm:grid-cols-2 gap-5"> {detailsList.map((item) => ( <DetailItem key={item.label} label={item.label} value={item.value} /> ))} {translation?.externalResources && ( <div className="border-l-4 border-cyan-500 pl-4 py-2.5 bg-gray-700/40 rounded-r-md shadow-sm"> <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Recursos Externos</p> <a href={translation.externalResources} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-base text-cyan-400 hover:text-cyan-300 hover:underline transition-colors break-all" > {getExternalLinkText(translation.externalResources)} <IconExternalLink className="w-4 h-4 ml-1.5 shrink-0"/> </a> </div> )} {translation?.tags && translation.tags.length > 0 && ( <div className="sm:col-span-2 border-l-4 border-cyan-500 pl-4 py-2.5 bg-gray-700/40 rounded-r-md shadow-sm"> <p className="flex items-center text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1"> <IconTag className="w-3.5 h-3.5 mr-1.5"/> Tags </p> <div className="flex flex-wrap gap-2 items-center mt-1.5"> {translation.tags.map(tag => ( <motion.span key={tag} className="text-sm bg-gray-600 text-gray-200 px-3 py-1 rounded-full cursor-default shadow-sm" whileHover={{ scale: 1.05, backgroundColor: 'rgb(103 232 249 / 0.3)'}} transition={{ duration: 0.2 }} > {tag} </motion.span> ))} </div> </div> )} </div> {detailsList.length === 0 && !translation?.externalResources && (!translation?.tags || translation.tags.length === 0) && ( <p className="text-gray-500 italic text-sm mt-4">No hay detalles técnicos disponibles.</p> )} </div> </motion.section>
                 {/* Sinopsis Section */}
                ...
                <motion.section variants={itemVariants} className="mt-16 pt-8 border-t border-gray-600">
                    ...
                    {disqusConfig && disqusConfig.identifier !== 'unknown_page' ? (
                        ...
                        // Show loading only while translation is loading, otherwise maybe "Comments disabled" or nothing
