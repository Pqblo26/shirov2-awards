...
// ReactMarkdown needed only if excerpt uses markdown
// import ReactMarkdown from 'react-markdown';
// import remarkGfm from 'remark-gfm';

// --- Interfaces (DownloadLink, SingleTranslationData) ---
interface DownloadLink { quality?: string; format?: string; server?: string; url: string; notes?: string; }
interface SingleTranslationData {
    slug: string; filename: string; title: string; date: string; imageUrl?: string;
    tags?: string[]; status?: string; mainCategory?: string; format?: string;
    source?: string; excerpt?: string; content: string; externalResources?: string;
    resolution?: string; videoCodec?: string; audioCodec?: string; downloads?: DownloadLink[];
}

// --- Simple SVG Icon Components (Definition remains the same) ---
...
function SingleTranslationPage() {
        ...
        const loadTranslation = async () => {
            ...
            try {
                ...
                const loadedTranslation: SingleTranslationData = {
                    ...
                    mainCategory: frontmatter.mainCategory, format: frontmatter.format, source: frontmatter.source,
                    excerpt: frontmatter.excerpt, content: markdownContent,
                    externalResources: frontmatter.externalResources, resolution: frontmatter.resolution,
                    videoCodec: frontmatter.videoCodec, audioCodec: frontmatter.audioCodec,
                    downloads: downloads,
                };
    ...
    const detailsList = [
        { label: "Fansubbing Work", value: translation.source },
        { label: "Recursos externos", value: translation.externalResources },
        { label: "Formato", value: translation.format },
        { label: "Resolución", value: translation.resolution },
        { label: "Video", value: translation.videoCodec },
        { label: "Audio", value: translation.audioCodec },
        ...
        >
            ...
            <motion.section variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-12">
                ...
                <div className="lg:col-span-2 bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-lg shadow-xl border border-gray-700/50 p-6">
                    {/* Section Title with Icon - SIZE ADDED */}
                    <h2 className="flex items-center text-xl font-semibold text-white mb-5 border-b border-cyan-500/30 pb-2">
                        <IconClipboardList className="w-5 h-5 mr-2 text-cyan-400"/> {/* <-- Added w-5 h-5 */}
                        Detalles Técnicos
                    </h2>
                         ...
                         {translation.tags && translation.tags.length > 0 && (
                             <div className="sm:col-span-2 border-l-4 border-cyan-500 pl-4 py-2.5 bg-gray-700/40 rounded-r-md shadow-sm">
                                  <p className="flex items-center text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">
                                      {/* Tag Icon - SIZE ADDED */}
                                      <IconTag className="w-3.5 h-3.5 mr-1.5"/> {/* <-- Added w-3.5 h-3.5 */}
                                       Tags
                                  </p>
                                  <div className="flex flex-wrap gap-2 items-center mt-1.5">
                                      {translation.tags.map(tag => (
                                          <motion.span
                                              key={tag}
                                              className="text-sm bg-gray-600 text-gray-200 px-3 py-1 rounded-full cursor-default shadow-sm"
                                              whileHover={{ scale: 1.05, backgroundColor: 'rgb(103 232 249 / 0.3)'}}
             ...
             {translation.excerpt && (
                <motion.section variants={itemVariants} className="mb-12 p-6 bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-lg shadow-xl border border-gray-700/50">
                     ...
                     <h2 className="flex items-center text-xl font-semibold text-white mb-3 border-b border-cyan-500/30 pb-2">
                         <IconBookOpen className="w-5 h-5 mr-2 text-cyan-400"/> {/* <-- Added w-5 h-5 */}
                         Sinopsis
                     </h2>
            ...
            <motion.section variants={itemVariants} className="mb-12 p-6 bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-lg shadow-xl border border-gray-700/50">
                 ...
                 <h2 className="flex items-center text-xl font-semibold text-white mb-4 border-b border-cyan-500/30 pb-2">
                     <IconDownload className="w-5 h-5 mr-2 text-cyan-400"/> {/* <-- Added w-5 h-5 */}
                     Descargas
                 </h2>
                 {translation.downloads && translation.downloads.length > 0 ? (
                     <div className="space-y-4">
                         {translation.downloads.map((link, index) => (
                             <motion.div
                                 key={index}
                                 className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-gray-700/60 rounded-lg border border-gray-600/50 shadow-sm"
                                 whileHover={{ backgroundColor: 'rgb(55 65 81 / 0.8)', scale: 1.01 }}
                                 transition={{ duration: 0.2 }}
                             >
                                 <div className="flex-1 mb-3 sm:mb-0 mr-4">
                                     <span className="font-semibold text-white text-base">
                                         {link.quality || 'Archivo'} {link.format ? `[${link.format}]` : ''} {link.server ? `- ${link.server}` : ''}
                                     </span>
                                 ...
                                 <motion.a
                                     ...
                                     className="inline-block bg-cyan-600 text-white text-sm font-bold py-2 px-5 rounded-md shadow-md whitespace-nowrap"
                                     whileHover={{ scale: 1.05, backgroundColor: 'rgb(8 145 178)', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1)'}}
                                     whileTap={{ scale: 0.95 }}
            ...
            <motion.section variants={itemVariants} className="mt-16 pt-8 border-t border-gray-600">
                 ...
                 <h3 className="flex items-center text-xl font-semibold text-white mb-4">
                     <IconChatBubbleLeft className="w-5 h-5 mr-2 text-gray-400"/> {/* <-- Added w-5 h-5 */}
                     Comentarios
                 </h3>
