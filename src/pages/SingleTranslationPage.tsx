...
interface SingleTranslationData {
    ...
    tags?: string[]; status?: string; mainCategory?: string; format_select?: string; format_other?: string;
    source_select?: string; source_other?: string; excerpt?: string; content: string; externalResources?: string;
    resolution_select?: string; resolution_other?: string; videoCodec_select?: string; videoCodec_other?: string;
    audioCodec_select?: string; audioCodec_other?: string; downloads?: DownloadLink[];
}
...
function SingleTranslationPage() {
    ...
    useEffect(() => {
        ...
        const loadTranslation = async () => {
            ...
            try {
                // --- MODIFICADO: Usar ruta relativa para el import dinámico ---
                // Desde src/pages/ subimos dos niveles (../../) hasta la raíz,
                // luego entramos a content/traducciones/
                ...
                // Actualizar mensaje de error si es fallo de import
                 if (err instanceof Error && (err.message.includes('Failed to fetch dynamically imported module') || err.message.includes('Unknown variable dynamic import'))) {
                     // Usar un mensaje más genérico ya que la causa raíz (ruta) debería estar corregida
                     setError(`No se pudo cargar el contenido para ${filename}. Verifica que el archivo existe y la ruta es correcta.`);
                 } else if (err instanceof Error && err.message.includes('Imported content is not a string')) {
                     setError(`Error: El contenido importado para /content/traducciones/${filename}.md no es texto.`);
                 } else {
                     setError("Traducción no encontrada o error al cargarla.");
                 }
    ...
    const DetailItem: React.FC<{ label: string; value: string | undefined | null }> = ({ label, value }) => { /* ... component code ... */
        ...
        return ( <div className="border-l-4 border-cyan-500 pl-4 py-2.5 bg-gray-700/40 rounded-r-md shadow-sm"> <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">{label}</p> <p className="text-base text-white">{value}</p> </div> );
     };
            ...
            <motion.section variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-12">
                ...
                <div className="lg:col-span-2 bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-lg shadow-xl border border-gray-700/50 p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                     ...
                     {detailsList.length === 0 && (!translation.tags || translation.tags.length === 0) && ( <p className="text-gray-500 italic text-sm mt-4">No hay detalles técnicos disponibles.</p> )}
                </div>
            </motion.section>
             ...
             {translation.excerpt && (
                <motion.section variants={itemVariants} className="mb-12 p-6 bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-lg shadow-xl border border-gray-700/50">
                     ...
                     <div className="prose prose-sm sm:prose-base prose-invert max-w-none prose-p:text-gray-300 prose-p:leading-relaxed"> <p>{translation.excerpt}</p> </div>
                </motion.section>
             )}
