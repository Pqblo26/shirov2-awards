...
function SingleTranslationPage() {
    ...
    // --- Prepare details list data (Removed externalResources) ---
    const detailsList = [
        { label: "Fansubbing Work", value: getDisplayValue(translation.source_select, translation.source_other) },
        // { label: "Recursos externos", value: translation.externalResources }, // REMOVED - Handled separately
    ...
    const DetailItem: React.FC<{ label: string; value: string | undefined | null }> = ({ label, value }) => {
        ...
        return ( <div className="border-l-4 border-cyan-500 pl-4 py-2.5 bg-gray-700/40 rounded-r-md shadow-sm"> <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">{label}</p> <p className="text-base text-white break-words">{value}</p> </div> ); // Added break-words
     };
    ...
    const getExternalLinkText = (url: string): string => {
        try {
            ...
            if (hostname.includes('myanimelist.net')) return "Ver en MyAnimeList";
            if (hostname.includes('anilist.co')) return "Ver en Anilist";
        } catch (e) {
            // Invalid URL? Fallback
        }
        return "Visitar Enlace Externo";
    };
    ...
    return (
        ...
        >
            ...
            <motion.section variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-12">
                ...
                <div className="lg:col-span-2 bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-lg shadow-xl border border-gray-700/50 p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        {/* Render standard details */}
                        {detailsList.map((item) => (
                            <DetailItem key={item.label} label={item.label} value={item.value} />
                        ))}

                        {/* --- Render External Resources Separately --- */}
                        {translation.externalResources && (
                             <div className="border-l-4 border-cyan-500 pl-4 py-2.5 bg-gray-700/40 rounded-r-md shadow-sm">
                                ...
                                <a
                                    href={translation.externalResources}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="inline-flex items-center text-base text-cyan-400 hover:text-cyan-300 hover:underline transition-colors break-all" // Added break-all
                                >
                     ...
                     {detailsList.length === 0 && !translation.externalResources && (!translation.tags || translation.tags.length === 0) && (
             ...
             {translation.excerpt && (
                <motion.section variants={itemVariants} className="mb-12 p-6 bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-lg shadow-xl border border-gray-700/50">
                     ...
                     {/* MODIFIED: Added whitespace-pre-wrap and removed prose div */}
                     <p className="text-gray-300 leading-relaxed whitespace-pre-wrap">
                         {translation.excerpt}
                     </p>
                     {/* If excerpt MIGHT contain markdown, use this instead:
                     <div className="prose prose-sm sm:prose-base prose-invert max-w-none prose-p:text-gray-300 prose-p:leading-relaxed">
                         <ReactMarkdown remarkPlugins={[remarkGfm]}>{translation.excerpt}</ReactMarkdown>
                     </div>
                     */}
                </motion.section>
             )}
            ...
            <motion.section variants={itemVariants} className="mb-12 p-6 bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-lg shadow-xl border border-gray-700/50">
                 ...
                 {/* ... Downloads rendering logic (remains the same) ... */}
