...
const previewTranslations = [
    {
        ...
        tag: "Manga",
        date: "5 abril, 2025", // Updated date
    ...
    },
    {
        ...
        tag: "Entrevista",
        date: "3 abril, 2025", // Updated date
     ...
     },
    {
        ...
        tag: "Noticia",
        date: "1 abril, 2025", // Updated date
...
interface FeatureCardProps {
    icon: React.ReactNode; // Accept SVG or other nodes
    title: string;
    description: string;
    link: string;
    color: 'pink' | 'blue' | 'green'; // Define specific colors for theme consistency
}
const FeatureCard: React.FC<FeatureCardProps> = ({ icon, title, description, link, color }) => {
    ...
    return (
        <motion.div
            variants={itemVariants} // Use variants defined in HomePage
...
// --- Reusable Card Component for Translation Previews ---
...
const TranslationPreviewCard: React.FC<TranslationPreviewProps> = ({ item }) => (
    // Using motion.div directly here for individual animation control if needed, or keep outer div
    <motion.div
        variants={itemVariants} // Apply item animation variant
        className="bg-gradient-to-tr from-gray-800/70 to-gray-900/80 rounded-xl shadow-lg overflow-hidden border border-gray-700/60 hover:border-green-500/60 transition-all duration-300 group flex flex-col sm:flex-row"
    >
        {/* Left: Image */}
        <div className="flex-shrink-0 sm:w-40 md:w-48"> {/* Fixed width for image */}
            <img
                src={item.imageUrl}
                ...
                onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/150x150/111827/9CA3AF?text=Error'; }} // Fallback
            />
        </div>
        {/* Right: Content */}
        <Link to={item.link} className="p-4 sm:p-5 flex flex-col flex-grow relative">
            ...
            <h4 className="text-base md:text-lg font-semibold mb-1.5 text-white group-hover:text-green-300 transition-colors pr-16">
             ...
             {/* Excerpt */}
            <p className="text-sm text-gray-300/90 flex-grow mb-3 line-clamp-2"> {/* Limit lines */}
                {item.excerpt}
            </p>
             {/* Read More */}
             <span className="text-xs text-green-400 mt-auto self-start group-hover:underline font-medium">
                Leer más &rarr;
             </span>
        </Link>
    </motion.div>
        ...
        >
            ...
            >
                ...
                <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
                    {/* Card: Premios */}
                    <FeatureCard
                        icon={ // Inline SVG Icon for Trophy
                    ...
                    />
                     {/* Card: Votaciones */}
                     <FeatureCard
                        icon={ // Inline SVG Icon for Check Square
                             <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    ...
                    />
                     {/* Card: Traducciones */}
                     <FeatureCard
                        icon={ // Inline SVG Icon for Book Open
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <motion.section
                className="py-16 md:py-20 px-6 bg-transparent" // Transparent background to blend
                variants={sectionVariants}
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true, amount: 0.1 }}
            >
                ...
                <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-1 gap-6">
                     {previewTranslations.map(item => (
                         <TranslationPreviewCard key={item.id} item={item} />
                     ))}
                </div>
            ...
            >
             ...
             <motion.section
                className="py-16 md:py-24 px-6 my-16 md:my-24 relative overflow-hidden"
                 variants={sectionVariants}
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true, amount: 0.2 }}
             >
                 ...
                 <div className="absolute inset-0 z-0 opacity-10">
                    <img src="https://placehold.co/1920x600/000000/333333?text=Anime+Montage+Background" alt="Anime background collage" className="w-full h-full object-cover"/>
                    <div className="absolute inset-0 bg-gradient-to-t from-gray-950 via-gray-950/80 to-transparent"></div> {/* Fade overlay */}
                 </div>
